...
@dataclass
class FusedMoEEvents310:
    before_routed_experts: Any
    before_dispatch: Any | None = field(default=None)
    before_combine: Any | None = field(default=None)
...
class AscendSharedFusedMoE310(SharedFusedMoE, AscendFusedMoE310):
    ...
    def _forward_shared_experts(self, hidden_states: torch.Tensor, fused_moe_evts: FusedMoEEvents310 | None = None):
        ...
        return self._finish_shared_experts(shared_out)
    ...
    def forward_impl(  # type: ignore[override]
        ...
        if overlap_shared_expert:
            ...
            # Submit the whole shared expert before enqueuing routed experts.
            # It only depends on hidden_states, so waiting for routed grouped
            # matmul artificially serializes swiglu/down_proj.
            shared_out = self._start_shared_experts(
                hidden_states,

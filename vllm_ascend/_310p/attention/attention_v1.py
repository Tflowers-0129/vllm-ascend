...
class AscendAttentionBackend310(AscendAttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        """
        if block_size % 16 != 0:
            raise ValueError(f"310P reshape_and_cache requires block_size aligned to 16, got {block_size}.")
        # Atlas inference products only support float16 on 310P. The ATB
        # ReshapeAndCache spec requires head_size to be 32-byte aligned, which
        # means 16 elements for fp16/bf16.
        if head_size % 16 != 0:
            raise ValueError(f"310P reshape_and_cache requires head_size aligned to 16, got {head_size}.")
